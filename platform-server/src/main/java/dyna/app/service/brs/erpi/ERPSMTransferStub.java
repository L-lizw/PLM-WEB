...
import dyna.app.service.brs.erpi.smIntegrateService.SMPLMServiceLocator;
import dyna.app.service.brs.erpi.smIntegrateService.SMPLMServiceSoap;
...
/**
 * @author lufeia
 * @date 2015-4-30 下午04:04:11
 * @return
 */
public class ERPSMTransferStub extends ERPTransferStub<ERPSMOperationEnum>
{
	...
	protected ERPSMTransferStub() throws Exception
	{
	...
	public BooleanResult checkConnection(ERPServiceConfig serverDef) throws Exception
	{
		...
		String returnString = this.checkConnectionCallWS(inputItem, ERPSMOperationEnum.PLMGETDSCMB);
	...
	private String checkConnectionString() throws ServiceRequestException, IOException
	{
		...
		doActionElement.addContent(ERPSMOperationEnum.PLMGETDSCMB.getNo());
	 ...
	 * @return
	 */
	private Element getNotifyEle(String erpFactory, String userId, ERPSMOperationEnum operation, String jobId)
	{
		ERPParameter param = this.integrate.getParameter(null);
		userId = StringUtils.isNullString(param.getParamMap().get("user")) ? ((IntegrateSM) this.integrate).SMUserId : param.getParamMap().get("user");
	...
	protected BooleanResult doExportWork() throws Exception
	{
		...
		try
		{
			...
			List<ERPSMOperationEnum> operationList = this.integrate.getOperationList(true);
			for (int i = 0; i < operationList.size(); i++)
			{
				this.startCount();
				this.currentOpeIdx = i;
				ERPSMOperationEnum operation = operationList.get(i);
				...
				if (totalCount <= 0)
				{
					...
					// continue;
					if (operation == ERPSMOperationEnum.PLMTODGBOM2)
					{
						String BOMOperationName = this.integrate.getOperationName(operation);
						operation = ERPSMOperationEnum.PLMToDGBOM;
						operation.setCategory(IntegrateERP.ERP_ITEM);
						operation.setName(this.stubService.getOperationAttribute(ERPServerType.ERPSM, this.integrate.getCreateItemOperationId()).get("name"));
						operationName = this.integrate.getOperationName(operation);
				...
				// 注意:集成方两方面:一是数据传输,二是数据在ERP中的处理结果。有的ERP是直接把两者合并一次传回（像E10和SM），这样PLM中的prepare4NextOperation就是一个空的实现
	...
	@Override
	protected String doCallWS(String originalXML, ERPSMOperationEnum operation) throws Exception
	{
	...
	@Override
	protected String checkConnectionDoCallWS(String originalXML, ERPSMOperationEnum operation) throws Exception
	{
	...
	@Override
	protected SMPLMServiceSoap getRemoteStub() throws MalformedURLException, ServiceException
	{
		if (this.stub == null)
		{
			this.stub = new SMPLMServiceLocator().getSMPLMServiceSoap(new URL(this.ERPConfig.getErpServerAddress()));
		}
		return (SMPLMServiceSoap) this.stub;
	}
	...
	@Override
	protected BooleanResult exportOpeation(ERPSMOperationEnum operation, int count) throws Exception
	{
		...
		{
			// 如果是传BOM，则在BOM传输最后通知SM BOM已经传输结束
	...
	public List<FoundationObject> getInfoFromERP(List<FoundationObject> list, ERPServiceConfig serviceConfig, List<String> factoryId) throws Exception
	{
		...
		List<ERPSMOperationEnum> operations = null;
		...
		{
			if (operations.contains(ERPSMOperationEnum.GETCOST) && operations.contains(ERPSMOperationEnum.GETPRICE))
			{
				operations.remove(ERPSMOperationEnum.GETCOST);
			}
			if (operations.size() > 1)
			{
				...
				{
					ERPSMOperationEnum operation = operations.get(i);
					this.currentOpeIdx += i;
	...
	private List<String> getcontents(List<String> contentList, ERPSMOperationEnum operation)
	{
		...
		if (!SetUtils.isNullList(contentList))
		{
			for (String content : contentList)
			{
				...
				map = this.getStubService().getERPStub().getContentAttribute(ERPServerType.ERPSM, content);
			...
			if (operation.equals(ERPSMOperationEnum.GETCOST) || operation.equals(ERPSMOperationEnum.GETPRICE))
			{
	...
	private void doExportOperation(boolean isbind, ERPSMOperationEnum operation, List<String> factoryId, List<FoundationObject> list, List<String> contentList) throws Exception
	{
		List<String> operationContent = null;
		if (operation.getId().equalsIgnoreCase(ERPSMOperationEnum.GETPRICE.getId()) || operation.getId().equalsIgnoreCase(ERPSMOperationEnum.GETCOST.getId()))
		{
		...
		else if (operation.getId().equalsIgnoreCase(ERPSMOperationEnum.GETQUANTITY.getId()))
		{
			if (isbind)
			{
				operationContent = this.getcontents(contentList, operation);
	 ...
	 * @throws JDOMException
	 */
	private void getQuantityReturn(String returnStringList, FoundationObject fo, String factory, ERPSMOperationEnum operation, List<String> contentList) throws JDOMException,
	 ...
	 * @throws Exception
	 */
	private void getPriceRreturn(boolean isbind, String returnStringList, List<FoundationObject> list, String factory, ERPSMOperationEnum operation, List<String> contentList)
			throws Exception
	{
		BooleanResult result = null;
		List<Element> resultElementList = null;
