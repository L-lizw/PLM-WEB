...
/**
 * @author lufeia
 * @date 2015-4-29 下午04:56:05
 * @return
 */
public class IntegrateSM extends IntegrateERP<ERPSMOperationEnum>
{
	...
	public final String		SMUserId	= "admin";

	public IntegrateSM(ERPTransferStub<ERPSMOperationEnum> stub, Document doc) throws Exception
	{
		super(stub, doc);
		this.xmlPath = "conf/smconf.xml";
	 ...
	 */
	@Override
	public Document getCreateDataXML(ERPSMOperationEnum operation, int totalCount, int index) throws Exception
	{
		Element stdEle = new Element("STD_IN");
		stdEle.addContent(this.getMsgHeadEleList(operation))
				.addContent(this.getDataEle(this.getHeaderPackageOfData(operation.getCategory()), operation.getCategory(), totalCount, index));
	...
	private List<Element> getMsgHeadEleList(ERPSMOperationEnum operation)
	{
		ERPParameter param = this.getParameter(null);
		List<Element> eleList = new ArrayList<Element>();
		eleList.add(new Element("Companyid").setText(this.stub.factory));
		String SMUser = param.getParamMap().get("user");
		if (StringUtils.isNullString(SMUser))
		{
			SMUser = this.SMUserId;
		}
		eleList.add(new Element("Userid").setText(SMUser));
	...
	private TableRecordData getLeafItemForBOM(BOMStructure struct, String category, ERPSMOperationEnum operation) throws ServiceRequestException
	{
	 ...
	 * @throws ServiceRequestException
	 */
	private TableRecordData getLeafBOMForBOM(BOMStructure struct, String category, ERPSMOperationEnum operation) throws ServiceRequestException
	{
		TableRecordData data = null;
		Iterator<String> tableIt = this.getTables(category).values().iterator();
		String tableName = null;
	 ...
	 * {@inheritDoc}
	 * <P/>
	 * SM提供的xml格式中，把局部取替代按照head-body(1-N)的关系，因此需要提供重载方法，并不能直接使用父类中方法
	 * <P>
	 * YT的xml是把每笔取替代做为一个RecordSet, 这种做法会产生主件和元件数据冗余问题
	 * 
	 * @see getEachLocalS4SM
	 */
	@Override
	@Deprecated
	public RecordSet getEachLocalS(FoundationObject obj, ERPSMOperationEnum operation) throws ServiceRequestException
	{
		throw new IllegalStateException("this method is not compatible with SMERP, please refers to 'getEachLocalS4SM' method");
	}
	 ...
	 * @throws ServiceRequestException
	 */
	private RecordSet getEachLocalS4SM(FoundationObject end1Obj, BOMStructure bomStructure, FoundationObject end2Obj, BOMTemplateInfo bomTemplate, ERPSMOperationEnum operation)
			throws ServiceRequestException
	{
		...
		String tableName = null;
		List<FoundationObject> substituteList = this.stub.getStubService().getBRM().listReplaceDataByRang(end1Obj.getObjectGuid(), end2Obj.getObjectGuid(), null, ReplaceRangeEnum.PART,
				ReplaceTypeEnum.TIDAI, bomTemplate.getName(), bomStructure.getBOMKey(), true, true);
	 ...
	 * 
	 * @see getEachLocalR
	 * @see getEachGlobalS4SM
	 */
	@Override
	@Deprecated
	public RecordSet getEachGlobalS(FoundationObject obj, ERPSMOperationEnum operation) throws ServiceRequestException
	{
		throw new IllegalStateException("this method is not compatible with SMERP, please refers to 'getEachGlobalS4WR' method");
	}

	private RecordSet getEachGlobalS4SM(FoundationObject end1Obj, BOMTemplateInfo bomTemplate, ERPSMOperationEnum operation) throws ServiceRequestException
	{
		if (this.checkObjectReturningSkip(end1Obj, ERP_REPLACESUBSTITUTE, operation))
		{
			return null;
		...
		while (tableIt.hasNext())
		{
			tableName = tableIt.next();
			TableRecordData tableRecord = new TableRecordData();
			List<FoundationObject> substituteList = this.stub.getStubService().getBRM().listReplaceDataByRang(null, end1Obj.getObjectGuid(), null, ReplaceRangeEnum.GLOBAL,
					ReplaceTypeEnum.TIDAI, null, null, true, true );
			if (substituteList != null && substituteList.size() != 0)
			{
				for (int j = 0; j < substituteList.size(); j++)
				{
	 ...
	 * 
	 * @see getEachLocalS
	 * @see getEachLocalR4SM
	 */
	@Override
	@Deprecated
	public RecordSet getEachLocalR(FoundationObject obj, ERPSMOperationEnum operation) throws ServiceRequestException
	{
		throw new IllegalStateException("this method is not compatible with SMERP, please refers to 'getEachLocalR4SM' method");
	}

	private RecordSet getEachLocalR4SM(FoundationObject end1Obj, BOMStructure bomStructure, FoundationObject end2Obj, BOMTemplateInfo bomTemplate, ERPSMOperationEnum operation)
			throws ServiceRequestException
	{
		if (this.checkObjectReturningSkip(end1Obj, ERP_REPLACESUBSTITUTE, operation))
		{
		...
		{
			tableName = tableIt.next();
			List<FoundationObject> substituteList = this.stub.getStubService().getBRM().listReplaceDataByRang(end1Obj.getObjectGuid(), end2Obj.getObjectGuid(),null,
					ReplaceRangeEnum.PART, ReplaceTypeEnum.QUDAI, bomTemplate.getName(), bomStructure.getBOMKey(), true, true);
			TableRecordData tableRecord = new TableRecordData();
			if (substituteList != null && substituteList.size() != 0)
			{
				for (int j = 0; j < substituteList.size(); j++)
	...
	@Override
	protected void getBOMData(FoundationObject end1Obj, List<BOMStructure> BOMStructureList, BOMStructure parentBOMStructure, ERPSMOperationEnum operation)
			throws ServiceRequestException
	{
		// SM在传BOM的时候要求把物料的简明信息也传过去，放在FormItem标签下
		...
		{
			this.addToDataMap(ERP_ITEM, this.getEachItemData(end1Obj, ERPSMOperationEnum.PLMToDGBOM));
		}
	...
	protected void getRSData(FoundationObject end1FoundationObj, List<BOMStructure> bOMStructureList, List<FoundationObject> end2FoundationObjList, BOMTemplateInfo bomTemplate,
			ERPSMOperationEnum operation) throws ServiceRequestException
	{
		...
		// 局部替代
		if (operation == ERPSMOperationEnum.PLMTOBOMI02)
		{
			for (int i = 0; i < end2FoundationObjList.size(); i++)
			{
				this.addToDataMap(ERP_REPLACESUBSTITUTE, this.getEachLocalS4SM(end1FoundationObj, bOMStructureList.get(i), end2FoundationObjList.get(i), bomTemplate, operation));
			}
		}
		// 局部取代
		else if (operation == ERPSMOperationEnum.PLMTOBOMI01)
		{
			for (int i = 0; i < end2FoundationObjList.size(); i++)
			{
				this.addToDataMap(ERP_REPLACESUBSTITUTE, this.getEachLocalR4SM(end1FoundationObj, bOMStructureList.get(i), end2FoundationObjList.get(i), bomTemplate, operation));
			}
		}
		// 全局替代
		else if (operation == ERPSMOperationEnum.PLMTOBOMI04)
		{
			// for (FoundationObject end2 : end2FoundationObjList)
			// {
			this.addToDataMap(ERP_REPLACESUBSTITUTE, this.getEachGlobalS4SM(end1FoundationObj, bomTemplate, operation));
			// }
		}
		// // 全局取代
		// else if (operation == ERPSMOperationEnum.PLMTOBOMI03)
		// {
		// for (FoundationObject end2 : end2FoundationObjList)
		// {
		// this.addToDataMap(ERP_REPLACESUBSTITUTE, this.getEachGlobalR4SM(end1FoundationObj, end2, bomTemplate,
		// operation));
		// }
		// }
	}
	...
	@Override
	public String getOperationId(ERPSMOperationEnum operation)
	{
		return operation.getId();
	}

	@Override
	public String getOperationCategory(ERPSMOperationEnum operation)
	{
		return operation.getCategory();
	}

	@Override
	public List<ERPSMOperationEnum> getOperationList(boolean isMergeByCategory) throws ServiceRequestException
	{
		List<String> operations = this.stub.schema.getOperationList();
		List<String> categoryList = new ArrayList<String>();
		List<ERPSMOperationEnum> operationList = new ArrayList<ERPSMOperationEnum>();
		for (int i = 0; i < operations.size(); i++)
		{
			String id = operations.get(i);
			Map<String, String> attrMap = this.stub.getStubService().getOperationAttribute(ERPServerType.ERPSM, id);
			ERPSMOperationEnum e = ERPSMOperationEnum.getById(id);
	...
	@Override
	public String getOperationCrossServiceName(ERPSMOperationEnum operation)
	{
		return operation.getWs();
	}

	@Override
	public String getOperationName(ERPSMOperationEnum operation)
	{
	...
	@Override
	public int getXMLPackageCount(ERPSMOperationEnum operation)
	{
	...
	@Override
	public String getReturnList(List<FoundationObject> list, String factory, ERPSMOperationEnum operation) throws Exception
	{
		String originalXML = null;
		this.stub.jobId = String.valueOf(System.nanoTime());
		if (operation.equals(ERPSMOperationEnum.GETPRICE) || operation.equals(ERPSMOperationEnum.GETCOST))
		{
			originalXML = this.getPriceOriginalXML(list, factory, operation);
		}
		else if (operation.equals(ERPSMOperationEnum.GETQUANTITY))
		{
	 ...
	 * @throws Exception
	 */
	private String getPriceOriginalXML(List<FoundationObject> list, String factoryId, ERPSMOperationEnum operation) throws Exception
	{
		ERPParameter param = this.getParameter(null);
		...
		if (!SetUtils.isNullList(list))
		{
			for (FoundationObject object : list)
			{
				...
				buffer.append(
						"<MB046> MB046</MB046><MB047> MB047</MB047> <MB051> MB051</MB051><MB057>MB057</MB057><MB058>MB058</MB058>  <MB059>MB059</MB059> <MB060>MB060</MB060></RecordList>");
			}
		}
	 ...
	 * @return
	 */
	private String getQuantityOriginalXML(List<FoundationObject> fo, String factory, ERPSMOperationEnum operation)
	{
		Date date = new Date();
		ERPParameter param = this.getParameter(null);
		StringBuffer buffer = new StringBuffer();
