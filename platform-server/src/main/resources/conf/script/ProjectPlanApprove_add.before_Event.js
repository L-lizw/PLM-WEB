...
{ 
	try{
		...
		if(isApproval != "Y")
		{
			return new new Packages.dyna.common.exception.ServiceRequestException("ID_APP_WF_PROJECT_NOT_APPROVAL", "project need not approval");	
		}
		
		var taskList = ppms.getWBSStub().listAllSubTask(objectGuid,false);

		if(!Packages.dyna.common.util.SetUtils.isNullList(taskList))
		{
			for(var j = 0 ; j < taskList.size(); j++ )
			{
				...
				if ("SUMMARY".equals(taskList.get(j).get("TaskType$name"))==false)
				{
					...
					{
					    return new new Packages.dyna.common.exception.ServiceRequestException("ID_APP_WF_PROJECT_NO_EXECUTOR", "task has not exist executor");	
					}
				}
			}
		}
